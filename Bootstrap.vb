  ...
  Namespace Control
    ...
    Public Class Control
      ...
      Public Sub New()
        ...
        Me.Region = Region.FromHrgn(Bootstrap.Control.Control.CreateRoundRectRgn(0, 0, Me.Width, Me.Height, Convert.ToInt32(Bootstrap.Control.Control.CalculateREM(Me) * 0.25), Convert.ToInt32(Bootstrap.Control.Control.CalculateREM(Me) * 0.25)))

        Me.BackColor = Bootstrap.Utilities.Color.BackgroundLight
        Me.ForeColor = Bootstrap.Utilities.Color.TextBody

        Me.Padding = New Padding(Convert.ToInt32(Bootstrap.Control.Control.CalculateREM(Me) * 0.75 * 72 / 96), Convert.ToInt32(Bootstrap.Control.Control.CalculateREM(Me) * 1.25 * 72 / 96), Convert.ToInt32(Bootstrap.Control.Control.CalculateREM(Me) * 0.75 * 72 / 96), Convert.ToInt32(Bootstrap.Control.Control.CalculateREM(Me) * 1.25 * 72 / 96))

        Me.BorderColor = Me.BackColor
        Me.DrawBorder = False
      End Sub

      Public Shared Function CalculateREM(ByVal control As Control) As Double
        Dim rootEM As Double = -1

        Using g As Graphics = Graphics.FromHwnd(control.Handle)
          rootEM = g.MeasureString("M", control.Font).Width
        End Using
      ...
      Protected Overrides Sub OnPaint(ByVal e As PaintEventArgs)
        ...
        Dim radius As Integer = Convert.ToInt32(Bootstrap.Control.Control.CalculateREM(Me) * 0.25)
      ...
      Protected Overrides Sub OnSizeChanged(ByVal e As EventArgs)
        ...
        Me.Region = Region.FromHrgn(CreateRoundRectRgn(0, 0, Me.Width, Me.Height, Convert.ToInt32(Control.CalculateREM(Me) * 0.25), Convert.ToInt32(Control.CalculateREM(Me) * 0.25)))
      End Sub
    End Class
      ...
      Protected Overrides Sub OnPaint(ByVal e As PaintEventArgs)
        ...
        If Me.Dismissable Then
          Using dismissBrush As Brush = New SolidBrush(styleConfiguration(Me.Style).Value)
            e.Graphics.DrawString(Bootstrap.Utilities.Typography.Close, Bootstrap.Utilities.Typography.Font, dismissBrush, New Point(Me.Width - Convert.ToInt32(Bootstrap.Control.Control.CalculateREM(Me)) - 5, 5))
        ...
        Using modal As Form = New Form()
          ...
          With modal
            ...
            AddHandler .Load, Sub(ByVal sender As Object, ByVal e As EventArgs)
                        modal.Region = Region.FromHrgn(Bootstrap.Control.Control.CreateRoundRectRgn(0, 0, modal.Width, modal.Height, Convert.ToInt32(Bootstrap.Control.Control.CalculateREM(_alert) * 0.25), Convert.ToInt32(Control.CalculateREM(_alert) * 0.25)))
                      End Sub
          End With
    ...
    Public Class Button
      ...
      Sub New()
        ...
        styleConfiguration.Add(Button.StyleType.Warning, New KeyValuePair(Of Drawing.Color, Drawing.Color)(Bootstrap.Utilities.BackgroundWarning, Bootstrap.Utilities.TextWhite))
